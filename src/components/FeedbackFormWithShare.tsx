...
const FeedbackFormWithShare = ({
  socialConfig: propSocialConfig,
  pageTitle: propTitle,
  onShare,
}: FeedbackFormWithShareProps) => {
  ...
  const socialConfig: SocialConfig =
    propSocialConfig || (config?.social as SocialConfig) || {};
  ...
  const isInstagramEnabled =
    socialConfig.enabled?.instagram !== false && !!socialConfig.instagram;
  const isFacebookEnabled =
    socialConfig.enabled?.facebook !== false && !!socialConfig.facebook;
  const isShareEnabled =
    socialConfig.enabled?.share !== false && !!socialConfig.shareUrl;
  ...
  const iconBgClass = isFooterVisible
    ? "bg-[#32D26C] hover:bg-[#2BC060]"
    : "bg-[#0F5C3D] hover:bg-[#0D4F34]";
  ...
  const {
    ...
    defaultValues: {
      type: "feedback",
      subject : "feeback",
      email : user?.email || "guest@yopmail.com",
    },
  });
  ...
  const onSubmit = async (data: FeedbackFormData, error) => {
    mutateAsync(data, {
      onSuccess: () => {
        toast({
          ...
          description: t(
            "feedbackSubmitDesc",
            `Your ${data.type} has been submitted. We'll review it soon.`,
          ),
        });
        reset();
        setOpen(false);
  ...
  const handleExternalLink = (
    ...
    const sanitizedUrl = trimmedUrl.startsWith("http")
      ? trimmedUrl
      : `https://${trimmedUrl}`;
          ...
          >
            {isInstagramEnabled && (
              <button
                type="button"
                onClick={(e) =>
                  handleExternalLink(e, socialConfig.instagram, "instagram")
                }
            ...
            {isFacebookEnabled && (
              <button
                type="button"
                onClick={(e) =>
                  handleExternalLink(e, socialConfig.facebook, "facebook")
                }
            ...
            {isShareEnabled && (
              <button
                type="button"
                onClick={(e) =>
                  handleExternalLink(e, socialConfig.shareUrl, "share")
                }
